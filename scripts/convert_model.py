...
def convert_torch2transformers_pocketllm(torch_path, transformers_path, dtype=torch.float16):
    PocketLLMConfig.register_for_auto_class()
    PocketLLMForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = PocketLLMForCausalLM(lm_config)
    ...
    print(f"模型已保存为 Transformers-PocketLLM 格式: {transformers_path}")
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    lm_model = PocketLLMForCausalLM(lm_config).to(device)
...
if __name__ == '__main__':
    lm_config = PocketLLMConfig(hidden_size=768, num_hidden_layers=8, max_position_embeddings=8192, use_moe=False)

    torch_path = os.path.join(ROOT_DIR, 'out', f"full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth")
    transformers_path = os.path.join(ROOT_DIR, 'minimind-3')
    convert_torch2transformers(torch_path, transformers_path)

    # # merge lora
    # base_torch_path = os.path.join(ROOT_DIR, 'out', f"full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth")
    # lora_path = os.path.join(ROOT_DIR, 'out', f"lora_identity_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth")
    # merged_torch_path = os.path.join(ROOT_DIR, 'out', f"merge_identity_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth")
    # convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path)

    # convert_transformers2torch(transformers_path, torch_path)
    # convert_json_to_jinja(os.path.join(MODEL_DIR, 'tokenizer_config.json'), os.path.join(MODEL_DIR, 'chat_template.jinja'))
    # convert_jinja_to_json(os.path.join(MODEL_DIR, 'chat_template.jinja'))
