...
def save_sft_checkpoint(epoch, step, swanlab=None):
    model.eval()
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            loss = res.loss + res.aux_loss  # 计算混合损失
            loss = loss / args.accumulation_steps  # 梯度累积缩小，避免梯度爆炸

        scaler.scale(loss).backward()  # 先进行缩放，然后再反向传播，避免因为半精度导致某些过小的梯度被忽略掉

        # 梯度裁剪
        if step % args.accumulation_steps == 0:
            scaler.unscale_(optimizer)  # 取消缩放，避免直接裁剪导致结果不对
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            if swanlab: swanlab.log({"loss": current_loss, "logits_loss": current_logits_loss, "aux_loss": current_aux_loss,
                                 "learning_rate": current_lr, "epoch_time": eta_min})

        # 模型保存
        if step % args.save_interval == 0 and step != iters:
            save_sft_checkpoint(epoch, step, swanlab)

        del input_ids, labels, res, loss

        # 最后一步如果没有正好累积完，也要更新一次参数，避免丢失最后的梯度
    ...
    if last_step > start_step:
        save_sft_checkpoint(epoch, last_step, swanlab)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PocketLLM Full SFT")
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/data/sft_t2t_mini.jsonl", help="预训练数据路径")
    ...
    parser.add_argument("--swanlab_project", type=str, default="PocketLLM-Full-SFT", help="swanlab项目名")
    ...
    # ========== 2. 配置目录、模型参数、检查ckp ==========
    os.makedirs(args.save_dir, exist_ok=True)
    lm_config = PocketLLMConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers,
    ...
    if args.use_swanlab:
        ...
        swanlab_run_name = f"PocketLLM-Full-SFT-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
        swanlab.init(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # 断点续训时，只在恢复的那个 epoch 跳过前面已经训练过的 step
        skip = start_step if (epoch == start_epoch and start_step > 0) else 0

        # 用普通索引构造 batch sampler
