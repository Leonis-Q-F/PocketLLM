...
def init_model(args):
    ...
    if 'model' in args.load_from:
        model = PocketLLMForCausalLM(PocketLLMConfig(
...
def main():
    parser = argparse.ArgumentParser(description="PocketLLM 模型推理与对话")
    parser.add_argument('--load_from', default='model', type=str, help="模型加载路径（model=原生torch权重，其他路径=transformers格式）")
    parser.add_argument('--save_dir', default='out', type=str, help="模型权重目录")
    parser.add_argument('--weight', default='full_sft', type=str, help="权重名称前缀（pretrain, full_sft, dpo, grpo 等）")
